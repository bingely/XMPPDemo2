...
public class IMService extends BaseService implements EventHandler,
		BackPressHandler {
	...
	public IBinder onBind(Intent intent) {
		L.i(IMService.class, "[SERVICE] onBind");
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		if (intent != null
			...
			String account = PreferenceUtils.getPrefString(IMService.this,
					PreferenceConstants.ACCOUNT, "");
			String password = PreferenceUtils.getPrefString(IMService.this,
		...
		mConnectingThread = new Thread() {
			...
			public void run() {
				try {
					...
					L.i(IMService.class, "YaximXMPPException in doConnect():");
					e.printStackTrace();
				} finally {
					if (mConnectingThread != null)
	...
	private void connectionFailed(String reason) {
		L.i(IMService.class, "connectionFailed: " + reason);
		...
		mMainHandler.post(new Runnable() {
			public void run() {
				if (!PreferenceUtils.getPrefBoolean(IMService.this,
					...
					MediaPlayer.create(IMService.this, R.raw.office).start();
				if (!isAppOnForeground())
					notifyClient(from, mSmackable.getNameForJID(from), message,
							!mIsBoundTo.contains(from));
				// T.showLong(IMService.this, from + ": " + message);
